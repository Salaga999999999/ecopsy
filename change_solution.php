...
<head prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb#">
    ...
    <title>Изменение предложения</title>
...
<?php $solutions = mysqli_query($connection, "SELECT * FROM `solutions` WHERE `id` LIKE " . $_GET["id"]);
    $solution = mysqli_fetch_assoc($solutions);
    ?>
<form action="change_solution_script.php" method="POST">
    <p style="color: black;">Название предложения:</p>
        <div class="field ">
            <input type="text" name="title" value="<?php echo $solution["title"];?>" required>
        </div><br>
        <p style="color: black;">В какую услугу поместить?</p>
        <select name="parant_id">
            <?php
            $services = mysqli_query($connection, "SELECT * FROM `menu__lists`");
            while($service = mysqli_fetch_assoc($services))
            {
            ?>
                <option value="<?php echo $service["id"];?>" <?php if ($service["id"] == $solution["parant_id"]){echo "selected";};?>><?php echo $service["RUS"];?></option>

            <?php
            }
            ?>
        </select><br><br>
        <p style="color: black;">Текст предложения</p>
        <div class="field  _textarea">
            <textarea type="text" name="text" required><?php echo $solution["text"];?></textarea>
        </div><br><br>
        <input type="hidden" value="<?php echo$_GET["id"];?>" name="id"/>
<div><input type="submit" name="button" value="Подтвердить изменение предложения" /></div> </br>
</form>

<form action="delete_solution_script.php" enctype="multipart/form-data" method="POST">
    <input type="hidden" value="<?php echo$_GET["id"];?>" name="id"/>
    <input type="submit"  name="button" value="Подтвердить удаление предложения" />
    </form>
